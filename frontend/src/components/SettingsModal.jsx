...
import { X, ShieldCheck, Key, Globe, CheckCircle2, AlertCircle, ExternalLink, Zap, Copy, Check, Users } from 'lucide-react';
import AssignmentSettingsTab from './AssignmentSettingsTab';

export default function SettingsModal({ isOpen, onClose }) {
  const defaultCallback = 'https://mailtracking-backend.onrender.com/api/auth/google/callback';
  const [activeTab, setActiveTab] = useState('gmail'); // 'gmail' | 'assignment'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div className="bg-[#111827] border border-slate-800 rounded-2xl w-full max-w-3xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">System Settings & Team Management</h2>
              <p className="text-xs text-slate-400">Configure Gmail API OAuth2, auto-assignment rules, & team members</p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 px-6 pt-3 bg-slate-950 border-b border-slate-800 text-xs">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 font-semibold rounded-t-xl transition-colors cursor-pointer border-b-2 ${activeTab === 'gmail' ? 'border-sky-500 text-sky-400 bg-slate-900/80' : 'border-transparent text-slate-400 hover:text-white'}`}
          >
            <ShieldCheck className="w-4 h-4" />
            <span>Gmail API & Mode</span>
          </button>

          <button
            onClick={() => setActiveTab('assignment')}
            className={`flex items-center gap-2 px-4 py-2.5 font-semibold rounded-t-xl transition-colors cursor-pointer border-b-2 ${activeTab === 'assignment' ? 'border-sky-500 text-sky-400 bg-slate-900/80' : 'border-transparent text-slate-400 hover:text-white'}`}
          >
            <Users className="w-4 h-4" />
            <span>Team Members & Auto-Rules</span>
          </button>
        </div>
        ...
        <div className="px-6 py-4 border-t border-slate-800 bg-slate-900/60 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={onClose} className="px-4 py-2 text-xs text-slate-400 hover:text-white cursor-pointer">
              Close
            </button>
            {activeTab === 'gmail' && (
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-5 py-2 bg-sky-500 hover:bg-sky-400 text-white text-xs font-semibold rounded-xl cursor-pointer"
              >
